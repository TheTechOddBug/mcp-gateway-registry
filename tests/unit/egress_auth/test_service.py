...
@pytest.mark.unit
class TestLegacyPrincipalFallback:
    """The egress vault principal id moved from the login username/email to the
    canonical OIDC ``sub`` (so browser-consent and bearer-vend agree across IdPs).
    Connections vaulted under the OLD id MUST still vend -- and migrate to the
    canonical id on read -- instead of stranding the user on "0 tools" until a
    manual re-consent. See ``get_valid_token(legacy_user_id=...)``.
    """

    _CANON = "sub-uuid-alice"
    _LEGACY = "alice@corp.com"

    async def test_legacy_hit_migrates_and_serves(self, svc, egress_oauth):
        # Vaulted under the legacy email principal ONLY (pre-cutover connection).
        await svc._store.put_token(
            "oauth2",
            self._LEGACY,
            "github",
        ...
        assert tok == "LEGACY"
        # Migrated: entry now lives under the canonical id and is gone from legacy.
        assert (
            await svc._store.get_token("oauth2", self._CANON, "github", "/github-mcp") is not None
        )
        assert await svc._store.get_token("oauth2", self._LEGACY, "github", "/github-mcp") is None

    async def test_canonical_hit_does_not_read_or_touch_legacy(self, svc, egress_oauth):
        await svc._store.put_token(
            "oauth2",
            self._CANON,
            "github",
            "/github-mcp",
            StoredToken(access_token="CANON", client_id="Iv1.testclient"),
        )
        await svc._store.put_token(
            "oauth2",
            self._LEGACY,
            "github",
            "/github-mcp",
            StoredToken(access_token="STALE", client_id="Iv1.testclient"),
        )
        tok = await svc.get_valid_token(
            "oauth2",
            self._CANON,
        ...
        assert tok == "CANON"
        legacy = await svc._store.get_token("oauth2", self._LEGACY, "github", "/github-mcp")
        assert legacy is not None and legacy.access_token == "STALE"

    async def test_no_legacy_and_equal_legacy_are_noop(self, svc, egress_oauth):
        assert await svc.get_valid_token("oauth2", self._CANON, "/github-mcp", egress_oauth) is None
        assert (
    ...
    async def test_migration_copy_failure_still_serves_legacy(self, svc, egress_oauth, monkeypatch):
        ...
        monkeypatch.setattr(svc._store, "put_token", boom)
        tok = await svc.get_valid_token(
            "oauth2",
            self._CANON,
            "/github-mcp",
    ...
    async def test_list_connections_merges_legacy_canonical_wins(self, svc):
        await svc._store.put_token(
            "oauth2", self._CANON, "github", "/a", StoredToken(access_token="1")
        )
        await svc._store.put_token(
            "oauth2", self._LEGACY, "github", "/b", StoredToken(access_token="2")
        )
        # Collision on /c: canonical must win, listed once.
        await svc._store.put_token(
            "oauth2", self._CANON, "github", "/c", StoredToken(access_token="C-new")
        )
        await svc._store.put_token(
            "oauth2", self._LEGACY, "github", "/c", StoredToken(access_token="C-old")
        )
        conns = await svc.list_connections("oauth2", self._CANON, legacy_user_id=self._LEGACY)
        assert sorted(c.server_path for c in conns) == ["/a", "/b", "/c"]

    async def test_disconnect_removes_both_principals(self, svc):
        await svc._store.put_token(
            "oauth2", self._CANON, "github", "/github-mcp", StoredToken(access_token="1")
        )
        await svc._store.put_token(
            "oauth2", self._LEGACY, "github", "/github-mcp", StoredToken(access_token="2")
        )
        await svc.disconnect(
            "oauth2", self._CANON, "github", "/github-mcp", legacy_user_id=self._LEGACY
        )
        assert await svc._store.get_token("oauth2", self._CANON, "github", "/github-mcp") is None
        assert await svc._store.get_token("oauth2", self._LEGACY, "github", "/github-mcp") is None
