...
An admission control webhook that can approve or deny registrations and updates BEFORE they are persisted. Unlike the notification webhook (which fires after the fact), the registration gate can block a request based on custom business logic such as naming conventions, compliance rules, or approval workflows. The gate is fail-closed: if the endpoint is unreachable after retries, registration is blocked. Sensitive fields (credentials, tokens, passwords) are automatically stripped from the payload sent to the gate.

[PR #881](https://github.com/agentic-community/mcp-gateway-registry/pull/881)
...
Fire-and-forget HTTP POST notifications when servers, agents, or skills are registered or deleted. Supports Bearer token and custom API key authentication. Webhook failures are logged but never block the operation. Useful for updating CMDBs, triggering CI/CD pipelines, or sending Slack notifications on registry changes.

[PR #878](https://github.com/agentic-community/mcp-gateway-registry/pull/878)
...
- Registration webhook notifications for add and delete events (#878)
- Registration gate admission control webhook with fail-closed design (#881)
